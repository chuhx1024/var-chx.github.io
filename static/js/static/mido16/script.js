$(function(){
  ...
  var mySwiper = new Swiper ('.swiper-container', {
    ...
    onTransitionStart: function(swiper){
      //你的事件
      if(swiper.realIndex>=1 && swiper.realIndex < 10){
        $('.nextpage').show();
      }else{
        $('.nextpage').hide();
      }
      if(swiper.realIndex>=2 && swiper.realIndex < 8){
        $('.deep_num').show();
        deepNum(swiper.realIndex);
      }else{
        $('.deep_num').hide();
      }
  ...
  function myBubble(){
    ...
    var setDemo1 = setInterval(function(){
      var idName = "img" + Math.ceil(Math.random()*17);
      demo.create({
...
var GOLBAL = {wxOptions:{title:"美度MIDO|腕表之家",desc:"美度MIDO|腕表之家",imgUrl:"http://www.xbiao.com/images/static/mido16/wx.jpg"}};
function doWx(){
